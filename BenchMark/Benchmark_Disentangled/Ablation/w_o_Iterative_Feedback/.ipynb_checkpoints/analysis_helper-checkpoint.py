...
def calculateDataPathwayOverlapGene(adata, customized_pathway=None, data_path=None):
    if customized_pathway is None:
        data_path = get_data_file_path(data_path,'gesa_pathways.npy')
    else:
...
def calculateTopPathwayGeneRanking(adata):
    ...
    for i in range(1, 4):
        ...
        for clusterid in set(stageadata.obs['leiden']):
            ...
            avg_geneWeightTable_ranking = np.zeros_like(avg_geneWeightTable_ranking) + len(
                adata_genes) - avg_geneWeightTable_ranking

            for pathway in list(pathway_gene.keys()):
                avg_ranking[i][clusterid][pathway] = 0
                for gene_in_pathway in pathway_gene[pathway]:
                    avg_ranking[i][clusterid][pathway] += avg_geneWeightTable_ranking[
                        adata_genes_index_dict[gene_in_pathway]]
    ...
    for i in list(avg_ranking.keys()):
        ...
        for j in list(avg_ranking[i].keys()):
            new_av_ranking[str(i)][str(j)] = {k: idx + 1 for idx, (k, v) in
                                              enumerate(sorted(avg_ranking[i][j].items(), key=lambda item: item[1]))}
...
def assign_drug_direction(adata, cmap_df=None, customized_drug_direction=None,data_path = None):
    # if cmap_df is not None:
    data_path = get_data_file_path(data_path,'brdID2cmapName.npy')
...
def find_overlap_and_assign_direction(adata, customized_direction=None, customized_drug=None, cmap_dir=None,data_path=None):
    load_cmap = False
    if customized_drug is None or customized_direction is None:
        if cmap_dir is None:
            raise ValueError(
                "cmap_dir must be provided! Please download the CMAPDirectionalDf.npy provided in the GitHub repo.")
    ...
    if customized_drug is None and customized_direction is None:
        ...
        if (customized_direction != None):
            ...
            adata = assign_drug_direction(
                adata, customized_drug_direction=customized_direction,data_path=data_path)
        else:
            print("using cmap drug direction")
            adata = assign_drug_direction(adata, cmap_df=cmap_df,data_path=data_path)

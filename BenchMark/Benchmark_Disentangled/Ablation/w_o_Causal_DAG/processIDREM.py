...
def getIdrem(paths, state):
    ...
    for i, each in enumerate(paths):
        joint_matrix = np.concatenate((state[0][each[0]].reshape(-1, 1), state[1][each[1]].reshape(-1, 1),
                                       state[2][each[2]].reshape(-1, 1), state[3][each[3]].reshape(-1, 1)), axis=1)
        out.append(joint_matrix)
    return out
...
def runIdrem(paths, midpath, idremInput, genenames, iteration, idrem_dir, species='Human',
    ...
    for i, each in enumerate(paths):
        ...
        examplefile_path = os.path.join(idrem_dir,
                                        'example_settings.txt')  # open(os.path.join(idrem_dir, 'example_settings.txt'), 'r')
                ...
                for k, line in enumerate(examplefile.readlines()):
                    ...
                    if trained and k == 4:
                        ...
                        f.write('%s\t%s\n' % ('Saved_Model_File', os.path.join(
                            os.path.abspath(os.path.join(midpath, str(iteration), 'idremInput')), f'{file_name}.txt')))
                    elif k == 1:
                        if species == 'Human':
...
if __name__ == '__main__':
    ...
    edges = [[[2, 0], [0, 1], [8, 3], [3, 4], [3, 5], [4, 6], [5, 7], [6, 8], [1, 9]],
             [[1, 0], [4, 1], [3, 2], [0, 3], [7, 5], [0, 6], [8, 7], [10, 10], [3, 11], [8, 12]],
             [[1, 0], [0, 1], [1, 2], [3, 3], [5, 4], [0, 5], [2, 6], [7, 8], [5, 9], [12, 10], [2, 12]]]

            ...
            for tf in stage:
                ...
                for each in enumerate(targetGenes):
                    ...
                    if each[1] in genedict.keys() and each[1] in scopes[i][j]:
                        ...
                        TFTG[i][j][
                            genedict[each[1]]] = 1 * foldChange + 1  # hyperparameter needed to be adusted by users
                        ...
                        # if TFTG[i][j][genedict[each[1]]]<2:
                        TFTG[i][j][
                            genedict[each[1]]] = 0.5 * foldChange + 0.5  # hyperparameter needed to be adusted by users
...
def updataGeneTablesWithDecay(mid, iteration, geneFactors, total_stage, decayRate=0.3):
    ...
    for i, stage in enumerate(tracks):
        if i != 0:
            ...
            adata.layers['geneWeight'] = adata.layers['geneWeight'].multiply(
                decayRate)  # sep8 change to all decrease for all cell

...
def auto_resolution(stagedata, anchor_index, neighbors, min_res, max_res):
    ...
    anchor_adata.obsp['connectivities'] = kneighbors_graph(anchor_adata.obsm['z'], neighbors[anchor_index],
                                                           mode='connectivity', include_self=True, n_jobs=20)
    anchor_adata.obsp['distances'] = kneighbors_graph(anchor_adata.obsm['z'], neighbors[anchor_index], mode='distance',
    ...
    for i in range(len(stagedata)):
        ...
        if i != anchor_index:
            temp_adata = stagedata[i]
            temp_adata.obsp['connectivities'] = kneighbors_graph(temp_adata.obsm['z'], neighbors[i],
                                                                 mode='connectivity', include_self=True, n_jobs=20)
            temp_adata.obsp['distances'] = kneighbors_graph(temp_adata.obsm['z'], neighbors[i], mode='distance',
                                                            include_self=True, n_jobs=20)

...
import scanpy as sc
import numpy as np
from .processIDREM import getClusterPaths, getClusterIdrem
...
def get_progressionmarker_background(times, adata, total_stage):
    ...
    for time in range(times):
        ...
        for i in range(len(stage_data)):
            ...
            tempX = stage_data[i].X.toarray() if hasattr(stage_data[i].X, 'toarray') else np.asarray(stage_data[i].X)

            # for j in range(stage_data[i].shape[1]):
            #     np.random.shuffle(tempX[:,j])
            np.random.shuffle(tempX)
            clusters = sorted(stage_data[i].obs['leiden'].astype(str).unique())
            for _cluster_index, cluster in enumerate(clusters):
                mask = stage_data[i].obs['leiden'].astype(str).to_numpy() == cluster
                avg_expression_val[i].append(np.mean(tempX[mask], axis=0))

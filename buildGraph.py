...
from .distDistance import calculateKL, getSimilarity, normalizeDistance
...
def connectNodes(distances, cutoff=0.05):
    ...
    for i in range(len(distances)):
        ...
        all_distances = np.asarray(distances, dtype=float).ravel()
        scale = max(float(np.std(all_distances)), 1e-12)
        pval = norm.cdf(distances[i][leftend], loc=float(np.mean(all_distances)), scale=scale)

        if pval < cutoff:
            edges.append([leftend, i])
    if not edges and distances:
        # Keep the best candidate so a valid adjacent-stage graph is always produced.
        best = min(
            ((float(distances[i][j]), j, i) for i in range(len(distances)) for j in range(len(distances[i]))),
            default=None,
        )
        if best is not None:
...
#     if len(edges) == 0:
#         raise RuntimeError(
#             'No edges found between stage %d and stage %d. Please adjust the number of neighbors or using a smaller clustering resolution.' % (
#             stage1, stage2))
#     return edges
...
def buildEdges(stage1, stage2, midpath, iteration, cutoff=0.05):
    ...
    reps = np.load(os.path.join(midpath, str(iteration) + '/rep.npy'), allow_pickle=True)

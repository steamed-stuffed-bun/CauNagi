...
from .distDistance import fitClusterGaussianRepresentation,calculateKL
...
def my_logfold_change(adata1, adata2,log_fold_change_cutoff=None,abs = False):
    ...
    x1 = adata1.X.toarray() if hasattr(adata1.X, "toarray") else np.asarray(adata1.X)
    x2 = adata2.X.toarray() if hasattr(adata2.X, "toarray") else np.asarray(adata2.X)
    mean1 = np.mean(x1, axis=0)
    mean2 = np.mean(x2, axis=0)
    logfold_change = mean2 - mean1
    order = np.argsort(np.abs(logfold_change) if abs else logfold_change)[::-1]
    df = pd.DataFrame({
        'log_fold_changes': logfold_change[order],
        'names': np.asarray(genenames)[order],
        'Mean_compared_clusters': mean1[order],
        'Mean_selected_clusters': mean2[order],
    })
...
def get_stage_hcmarkers(adata,key,use_rep,cutoff=0.05):
    ...
    elif use_rep in {'umaps', 'umap', 'X_umap'}:
        mydendrogramUMAP(adata,use_rep='X_umap',groupby=key)
    else:
        raise ValueError("use_rep must be 'z' or an available UMAP representation.")
...
def get_dataset_hcmarkers(adata,stage_key,cluster_key,use_rep,cutoff=0.05):
    ...
    for each in list(adata.obs[stage_key].unique()):
        ...
        if stage_adata.obs[cluster_key].nunique() < 2:
            hcmarkers[str(each)]['Z'] = np.empty((0, 4))
            hcmarkers[str(each)]['order'] = [str(stage_adata.obs[cluster_key].iloc[0])] if len(stage_adata) else []
            hcmarkers[str(each)]['markers'] = {}
            continue

...
# Respect a user-provided device selection before importing torch.
...
from .utils import create_activation, create_norm, mean_flat
...
def loss_backwards(fp16, loss, optimizer, **kwargs):
    if fp16:
        try:
            ...
            raise ImportError("NVIDIA Apex is required for fp16 training.") from exc
...
class Denoise_net(nn.Module):
    ...
    def forward(self, x, x_start, time, labels=None, concept_embs=None, gene_weights=None):
        # if self.cond_embed is not None:
        #     cond_emb = self.cond_embed(conditions)[0]
        #     x = x + cond_emb.squeeze(1)
        if labels is not None and concept_embs is None:
            concept_embs, mask_recon_loss, pred_o_loss, discriminator_loss, prior_kl = self.DisentanglementEncoder(
                x_start, labels)
            ...
            raise ValueError("Provide either labels or concept_embs, but not both.")
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance_with_factor(self, x, t, concept_embs, clip_denoised: bool, eps=False):
        ...
        if eps:
            x_recon = self.predict_start_from_noise(x, t=t,
                                                    noise=self.denosie_fn(x, x_start, t, concept_embs=concept_embs))
        else:
    ...
    def p_sample_with_factor(self, x, t, concept_embs, clip_denoised=False, repeat_noise=False):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance_with_factor(x=x, t=t, concept_embs=concept_embs,
    ...
    def p_losses(self, x_start, t, labels, weights, noise=None, eps=False, gene_weights=None):
        b, c = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_recon, mask_recon_loss, pred_o_loss, discriminator_loss, prior_kl = self.denosie_fn(x_noisy, x_start, t,
                                                                                              labels)

        assert x_recon.shape == x_noisy.shape, "Please check the code and data"

        if gene_weights is not None:
            # Keep every gene in the objective and emphasize accumulated evidence.
            gene_weights = 1.0 + gene_weights.clamp_min(0.0)
            if self.loss_type == "l1":
                if eps:
                    loss = ((noise - x_recon).abs() * gene_weights).mean()
                else:
                    loss = ((x_start - x_recon).abs() * gene_weights).mean()
            elif self.loss_type == "l2":
                if eps:
                    loss = ((noise - x_recon)**2 * gene_weights).mean()
                else:
                    loss = ((x_start - x_recon)**2 * gene_weights).mean()
            else:
                raise NotImplementedError(f"Unsupported loss type: {self.loss_type}")
        else:
            if self.loss_type == "l1":
                if eps:
                    loss = (((noise - x_recon).abs()) * weights[:, None]).mean()
                else:
                    loss = (((x_start - x_recon).abs()) * weights[:, None]).mean()
            elif self.loss_type == "l2":
                if eps:
                    loss = (((noise - x_recon) ** 2) * weights[:, None]).mean()
                else:
                    loss = (((x_start - x_recon) ** 2) * weights[:, None]).mean()
            else:
...
class DisentanglementEncoder(nn.Module):
    def __init__(self,
        ...
        causal_dag = torch.as_tensor(causal_dag, dtype=torch.float32)
        if causal_dag.shape != (num_factor, num_factor):
            raise ValueError("causal_dag must be a square matrix matching num_factor.")
        self.register_buffer("causal_dag", causal_dag)
        self.register_buffer("I", torch.eye(num_factor))
        ...
        self.label_predictor = nn.ModuleList(
            [nn.Linear(out_dim, num) for num in label_categories]
        )
    ...
    def forward(self, x, o):
        exogenous_factor_m, exogenous_factor_v = torch.split(self.exogenous_encoder_m_v(x),
                                                             self.num_factor * self.out_dim, dim=-1)
        ...
        z = torch.linalg.solve(self.I - self.causal_dag, exogenous_embs)
        ...
        pred_o_loss /= len(pred_o)

        # Train the unexplained concept adversarially to reduce label leakage.
        pred_u = []
        for idx, predictor in enumerate(self.label_predictor):
        ...
        # take mean-level loss
        discriminator_loss = - pred_u_loss / len(pred_u)

        return concept_embs, mask_recon_loss, pred_o_loss, discriminator_loss, prior_kl

    def extract_exogenous_embs(self, x):
        with torch.no_grad():
            exogenous_factor_m, exogenous_factor_v = torch.split(self.exogenous_encoder_m_v.eval()(x),
                                                                 self.num_factor * self.out_dim, dim=-1)
            exogenous_factor = self.sample(exogenous_factor_m, exogenous_factor_v)
            exogenous_embs = rearrange(exogenous_factor, 'b (h d) -> b h d', h=self.num_factor)
    ...
    def forward(self, x, *, context=None, mask=None):
        ...
        if exists(mask):
            ...
            if mask.ndim == 2:
                ...
                raise ValueError("Attention masks must have two or three dimensions.")

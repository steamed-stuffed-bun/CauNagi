...
def mcSampling(mus, sigmas):
    ...
    if mus.ndim != 2 or sigmas.shape != mus.shape:
        raise ValueError("mus and sigmas must have shape (cells, features).")
    samplegaussian = [[] for _ in range(mus.shape[1])]
    hiddensize = mus.shape[1]
    mus = np.vstack(mus)
    ...
    for i in range(hiddensize):
        ...
        std = np.maximum(sigmas[:, i], 1e-6)
        std = std.repeat(100)
        normal = Normal(torch.zeros(size=mean.shape) + torch.tensor(mean),
                        torch.zeros(size=std.shape) + torch.tensor(std)).sample()
...
def normalizeDistance(distance):
    ...
    if distance.ndim != 2 or distance.shape[1] != 2:
        raise ValueError("distance must have shape (n_pairs, 2).")
    ...
    if len(gaussiankld) == 1:
        return gaussiankld + de
    de_minmax = minmax(de)
    ggkld_minmax = minmax(gaussiankld)
...
def calculateKL(cluster1gaussian, cluster2gaussian):
    ...
    if cluster1.shape != cluster2.shape:
        raise ValueError("Gaussian representations must have the same dimension.")
    std1 = np.maximum(cluster1[:, 1], 1e-6)
    ...
    mean2 = cluster2[:, 0]
    std2 = np.maximum(cluster2[:, 1], 1e-6)

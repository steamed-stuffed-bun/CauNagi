...
def getTargetGenes(path, N):
    ...
    for each in filenames:
        if each[0] != '.':
            ...
            out.append(getPosNegDynamicPathGenes(path, each,
                                                 N))  # get the genes from nodes increase and descrease most between stages
    return out
...
def matchTFandTGWithFoldChange(TFs, scopes, avgCluster, filename, genenames, total_stage):
    ...
    genedict = {str(each).upper(): i for i, each in enumerate(genenames)}
    ...
    for i, track in enumerate(TFs):
        ...
        for j, stage in enumerate(track):
            ...
            for tf in stage:
                ...
                targetGenes = TG.get(tf[0].split(' ')[0], [])
                for each in scopes[i][j]:
                    foldChange = abs(np.log2(avgCluster[j + 1][j][genedict[each]] + 1) - np.log2(
                        avgCluster[j][i][genedict[each]] + 1))
                    TFTG[i][j][genedict[each]] = 1 * foldChange + 1
                temp2 = np.zeros(shape=len(genenames))
                for each in enumerate(targetGenes):

                    target_gene = each[1].upper()
                    scope_genes = {gene.upper() for gene in scopes[i][j]}
                    if target_gene in genedict and target_gene in scope_genes:

                        foldChange = abs(np.log2(avgCluster[j + 1][j][genedict[each[1]]] + 1) - np.log2(
                            avgCluster[j][i][genedict[each[1]]] + 1))
                        # if TFTG[i][j][genedict[each[1]]]<2:
                        TFTG[i][j][
                            genedict[target_gene]] = 1 * foldChange + 1
                    elif target_gene in genedict:

                        foldChange = abs(np.log2(avgCluster[j + 1][i][genedict[each[1]]] + 1) - np.log2(
                            avgCluster[j][i][genedict[each[1]]] + 1))

                        # if TFTG[i][j][genedict[each[1]]]<2:
                        TFTG[i][j][
                            genedict[target_gene]] = 0.5 * foldChange + 0.5

                tf_name = tf[0].split(' ')[0].upper()
                if tf_name in genedict:
                    index = genedict[tf_name]
...
def updateGeneFactorsWithDecay(adata, clusterid, iteration, geneWeight, decayRate=0.5):
    ...
    if 'geneWeight' not in adata.layers.keys():
        ...
        current_weight = adata.layers['geneWeight']
        adata.layers['geneWeight'] = current_weight.tolil() if issparse(current_weight) else lil_matrix(current_weight)
    adata.layers['geneWeight'][celllist] += geneWeight
    adata.layers['geneWeight'] = adata.layers['geneWeight'].tocsr()
    return adata


def updateGeneTablesWithDecay(mid, iteration, geneFactors, total_stage, decayRate=0.3):
    '''
    ...
    '''
    if total_stage < 2:
        raise ValueError("At least two stages are required to update gene weights.")
    ...
    for i, stage in enumerate(tracks):
        if i != 0:
            ...
            if 'geneWeight' not in adata.layers:
                adata.layers['geneWeight'] = csr_matrix(np.zeros(adata.shape, dtype=np.float32))
            temppreviousMySigmoidGeneWeight = adata.layers['geneWeight']
            if not issparse(temppreviousMySigmoidGeneWeight):
                temppreviousMySigmoidGeneWeight = csr_matrix(temppreviousMySigmoidGeneWeight)
            adata.layers['geneWeight'] = temppreviousMySigmoidGeneWeight.multiply(decayRate)
...
# Keep the historical misspelling as a compatibility alias.
updataGeneTablesWithDecay = updateGeneTablesWithDecay

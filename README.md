# CauNagi
**CauNagi** is a framework for causal representation learning and temporal regulatory analysis of multistage single-cell transcriptomic data. It disentangles predefined biological concepts, learns diffusion-based cell representations, identifies stage-specific cell populations, constructs temporal relationships, and uses iDREM to infer dynamic transcription factor–target regulatory programs during disease progression. Its downstream modules prioritize candidate regulatory and driver genes. The **Candidate_Regulator_Screening** module integrates iteratively updated geneWeight-based scores, iDREM-derived TF activity, dynamic and hierarchical markers, network topology, and cell-type-specific trajectories to rank genes using a weighted driver_score. The **cascade_classification** module evaluates cross-cell-type specificity, propagation patterns, and layer consistency to classify candidates as initiators, propagators, amplifiers, or shared cascade-core genes, thereby identifying candidate key regulators of disease progression.
